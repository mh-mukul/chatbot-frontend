  ...
  (
    ...
    const [_open, _setOpen] = React.useState(() => {
      ...
      const cookieValue = document.cookie
        .split(";")
        .find((row) => row.trim().startsWith(`${SIDEBAR_COOKIE_NAME}=`))
      if (cookieValue) {
    ...
    if (isMobile) {
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          ...
          >
            <VisuallyHidden.Root>
              <SheetTitle>Sidebar</SheetTitle> {/* Hidden title for accessibility */}
            </VisuallyHidden.Root>
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Hard-coded width to prevent hydration error from random values.
  const width = "85%";
